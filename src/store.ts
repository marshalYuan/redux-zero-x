...
function getActions(store: IStore) {
    ...
    return propertyKeys.filter(propertyKey => isFunction(store[propertyKey]))
              .reduce((actions, propertyKey) => {
                  ...
                  actions[name] = (...args) => {
                    ...
                    return hasMiddleware ? store.middleware(action, () => set(store, store[propertyKey](...args)))
...
export class Store implements IStore {
    ...
    constructor(protected state: any = {}, ...middlewares) {
         this.middleware = (Store.middlewares.length || middlewares.length) ? compose([...Store.middlewares, ...middlewares]) : null
    }
